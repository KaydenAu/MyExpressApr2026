...
router.get('/', async(req, res)=>{
    try{
        ...
        errorResponse(res, 500, "Server error. Failed to retrieved data", err.message);
    }
});
...
router.get('/:id', async(req, res)=>{
    try{
        ...
        successResponse(res, 200, "Successful", detailReport)
    } catch(error){
...
router.put('/update/:id', upload.single('image'), async(req, res) =>{
    ...
    errors = validatePayload(title, date, category);
    if(errors?.length > 0) return errorResponse(res, 400, 'Validation Failed', null, errors);
